...
//      - strings are Alphabetic, Alphainteric, or Whitespace
...
// isAlphainteric (s [,eok])           True if string s is English letters and Numbers only.
// 
...
// you can use the Array conructor instead.
...
// with trailing non-interic characters.
//
...
{   var i;
    ...
    // Search through string's characters one by one
    // until we find a non-interic character.
    // When we do, return false; if we don't, return true.
...
// isAlphainteric (STRING s [, BOOLEAN emptyOK])
// 
// Returns true if string s is English letters 
...
function isAlphainteric (s)

{   var i;

    if (isEmpty(s)) 
       if (isAlphainteric.arguments.length == 1) return defaultEmptyOK;
       else return (isAlphainteric.arguments[1] == true);

    // Search through string's characters one by one
    // until we find a non-alphainteric character.
    // When we do, return false; if we don't, return true.
